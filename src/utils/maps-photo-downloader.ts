...
function extractEmbeddedImageUrl(mapsUrl: string): string | null {
  try {
    ...
    const m = decoded.match(/https?:\/\/lh3\.googleusercontent\.com\/[^\s"'&]+/i);
    if (m) return m[0];
    return null;
  } catch (err) {
    return null;
 ...
 */
async function downloadImageToStore(imageUrl: string): Promise<{ filename: string; publicPath: string }> {
  await ensureStoreDir();
  // follow redirects and stream
  const res = await axios.get(imageUrl, { responseType: "stream", maxRedirects: 5, timeout: 20000 });
 ...
 */
async function downloadPhotoViaPlaces(placeQueryOrMapsUrl: string): Promise<{ filename: string; publicPath: string } | null> {
  if (!GOOGLE_API_KEY) {
    console.warn('GOOGLE_MAPS_API_KEY not configured — skipping Places API photo fetch');
    return null;
  }
  ...
  try {
    console.log('[maps-downloader] calling FindPlaceFromText for:', placeQueryOrMapsUrl);
    ...
    console.log('[maps-downloader] FindPlaceFromText response status:', findRes.status);
    const candidates = findRes.data?.candidates ?? [];
    console.log('[maps-downloader] candidates.length =', Array.isArray(candidates) ? candidates.length : 'not-array');
    if (Array.isArray(candidates) && candidates.length > 0) {
      // prefer candidate with photos
      const candidateWithPhoto = candidates.find((c: any) => Array.isArray(c.photos) && c.photos.length > 0) || candidates[0];
      const placeId = candidateWithPhoto.place_id;
      console.log('[maps-downloader] selected placeId:', placeId, 'hasPhotos:', Array.isArray(candidateWithPhoto.photos) && candidateWithPhoto.photos.length>0);
      ...
      const photos = detailsRes.data?.result?.photos || candidateWithPhoto.photos || [];
      console.log('[maps-downloader] photos.length =', Array.isArray(photos) ? photos.length : 'not-array');
      if (photos && photos.length > 0) {
        const best = photos[0];
        const photoRef = best.photo_reference;
        console.log('[maps-downloader] photo_reference:', photoRef ? '[present]' : '[missing]');
        if (photoRef) {
          ...
          const photoRes = await axios.get(photoUrl, {
            params: { photoreference: photoRef, maxwidth: 1600, key: GOOGLE_API_KEY },
            responseType: "stream",
            maxRedirects: 5,
            timeout: 20000,
          });
 ...
 */
export async function downloadFirstPhotoFromMapsLink(mapsUrlOrQuery: string): Promise<{ filename: string; publicPath: string } | null> {
  ...
  try {
    ...
    if (embedded) {
      try {
        ...
        console.warn("Failed to download embedded image URL:", err?.message || err);
    ...
    const placeName = extractPlaceNameFromMapsUrl(mapsUrlOrQuery) || mapsUrlOrQuery;
